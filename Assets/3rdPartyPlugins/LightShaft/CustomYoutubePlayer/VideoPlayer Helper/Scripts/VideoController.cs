...
namespace Unity.VideoHelper
{
    ...
    public class VideoController : MonoBehaviour
    {
        ...
        public VideoPresenter _controller;
        ...
        public void OnClickPlay()
        {
            if (StartAfterPreparation)
                Play();
            _controller.Thumbnail.gameObject.SetActive(false);
            playButton.SetActive(false);
        }

        public async void DownloadAndLoad(string prefix, string url)
        {
            _controller.LoadingIndicator.gameObject.SetActive(true);
            await Services.DownloadVideo(prefix,url, (localUrl)=> {
                //Debug.LogError("Start Video Dowload Compelte. : "+ localUrl);
                videoPlayer.source = VideoSource.Url;
                videoPlayer.url = localUrl;
                videoPlayer.Prepare();
            });
            _controller.LoadingIndicator.gameObject.SetActive(false);
        }
        ...
        {
            DurationStart = (ulong)(videoPlayer.frameCount / videoPlayer.frameRate);

            onPrepared.Invoke();
            
            if (!string.IsNullOrEmpty(TrailsHandler.instance.CurrentTrailPoi.standard_video_file_url))
            {
                Seek(0.5f);
                videoPlayer.Play();
                thumb.texture = videoPlayer.texture;
                videoPlayer.Pause();
                videoPlayer.playbackSpeed = 1;
                thumb.SetGameObjectActive(true);
